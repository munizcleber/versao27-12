...
import { AnyMediaMessageContent, proto, WASocket } from "@whiskeysockets/baileys";
  ...
  if (showChatBots.options) {
    ...
    const botText = async () => {
      ...
      if (choosenQueue.optionType === 'file') {
        ...
        let optionsMessage = {} as AnyMediaMessageContent;
        if (mime.includes('image')) {
          optionsMessage = {
            image: fs.readFileSync(publicFolder + '/company' + ticket.companyId + '/' + choosenQueue.greetingMessage),
          ...
          optionsMessage = {
            document: fs.readFileSync(publicFolder + '/company' + ticket.companyId + '/' + choosenQueue.greetingMessage),
            caption: `*[ # ]* - Voltar Menu Inicial`,
            fileName: choosenQueue.greetingMessage.toLocaleLowerCase(),
            mimetype: mime
          };
