...
const verifyQueue = async (
  ...
  const botText = async () => {
    ...
    const sendMediaGreetingAtachment = async() => {
      const medias = JSON.parse(whatsapp.greetingMediaAttachment);
      for (const media of medias) {
        try {
          const filePath = path.resolve(`public/company${ticket.companyId}`, media);
          const messagePath = media;
          const options = await getMessageOptions(messagePath, filePath, ticket.companyId.toString());

          let sentMessage = await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, { ...options });
          await verifyMessage(sentMessage, ticket, ticket.contact);
        } catch (error) {
          console.log('Erro no envio do arquivo ' + media, error);

        }
      }
    }
    if(whatsapp.greetingMediaAttachment !== null) {
      await sendMediaGreetingAtachment()
    }
