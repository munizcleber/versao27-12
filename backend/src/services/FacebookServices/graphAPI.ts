...
export const profilePsid = async (id: string, token: string): Promise<any> => {
  try {
    const { data } = await axios.get(
      `https://graph.facebook.com/v16.0/${id}?access_token=${token}`
    );
    return data;
  } catch (error) {
    console.log(error)
...
export const subscribeApp = async (id: string, token: string): Promise<any> => {
  try {
    const { data } = await axios.post(
      `https://graph.facebook.com/v16.0/${id}/subscribed_apps?access_token=${token}`,
      {
        subscribed_fields: [
          "messages",
          "messaging_postbacks",
...
export const removeApplcation = async (
  ...
  try {
    const { data } = await axios.delete(
      `https://graph.facebook.com/v16.0/${id}/permissions`,
      {
        params: {
          access_token: token
        }
